...
<?php
	...
	else
	{
		...
		if(!$b)
			...
			$sql = "SELECT Punctaj, Nume, Poza FROM masini ORDER BY Punctaj DESC";
...
if (mysqli_num_rows($result) > 0) {
  ...
  while($row = mysqli_fetch_assoc($result)) {
	  if ($s=="1")
	  {
		  echo '<div class="row1">';
	echo  '<div class="name">';
	echo '<img src="data:image/png;base64,'.base64_encode($row["Poza"]).'" width="350px" height="100px" align="left""/>'." ";
	echo $row["Nume"];
	echo '</div>';
